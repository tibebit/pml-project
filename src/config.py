...
def get_feature_set(name: str = DEFAULT_FEATURE_SET_NAME) -> list[str]:
    try:
        return list(FEATURE_SET_CANDIDATES[name])
    except KeyError as exc:
        known = ", ".join(sorted(FEATURE_SET_CANDIDATES))
        raise ValueError(f"Unknown feature set {name!r}. Known feature sets: {known}") from exc
